//! Demonstrates driving a [`BoundaryAwareCodec`] directly, call by call,
...
fn drives_three_segments_across_two_early_stops() {
    ...
    let progress = codec
        ...
        .unwrap();
    let BoundaryAwareProgress::Boundary { consumed, written } = progress else {
    ...
    let progress = codec
        .process(&input[pos..], as_uninit_mut(&mut output))
        .unwrap();
    let BoundaryAwareProgress::InputConsumed { written } = progress else {
...
/// into a `Source` in its own right. Built on [`Pump`]/`boundary_aware_pump_read`,
...
impl<I: Source, C: BoundaryAwareCodec, const N: usize> Source for CodecSource<I, C, N> {
    type Error = rust_codecs_core::DriveError<I::Error, Infallible>;

    fn chunk(&mut self) -> Result<Option<&[u8]>, Self::Error> {
        if self.pos == self.len {
            self.len = rust_codecs_core::sources_and_sinks::shared_io::boundary_aware_pump_read(
                &mut self.pump,
                &mut self.inner,
                &mut self.buf,
            )?;
